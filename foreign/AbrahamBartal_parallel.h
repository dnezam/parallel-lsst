...
public:
  AbrahamBartal_par(ListGraph &G, ListGraph::EdgeMap<double> &l)
      : Gr(G), length(l) {
    c = 2;
    N = countNodes(G);
    beta =
  ...
  void run(ListGraph::Node &x0, ListGraph &Tree,
    ...
    FirstHierarStarPart(Gr, x0, length, Q, Tree, lengthTNew, TNewId, TOldId);
  }

  void FirstHierarStarPart(ListGraph &G, ListGraph::Node x0,
