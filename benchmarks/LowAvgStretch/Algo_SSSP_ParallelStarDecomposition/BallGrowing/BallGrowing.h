...
namespace gbbs
{
    namespace LowAvgStretch
    {
                               ...
                               sequence<std::tuple<uintE, uintE, typename Graph::weight_type>> &edges) // sequence<PPD> &parents)
        {
            ...
            // alternative way of creating radius array:
            // radius = sequence<size_t>::uninitialized(srcs.size());
            // parallel_for(0, radius.size(), [&](size_t i) {
            //     radius[i] = *(
            //         parlay::max_element(
            //             parlay::remove_if(
            //                 sequence<size_t>::from_function(G.n, [&](size_t j) {
            //                     if (cluster[j] == cluster[srcs[i]]) {
            //                         return dists[j];
            //                     }
            //                     return SIZE_MAX;
            //                 }),
            ...
            //     );
            // });
            // std::cout << "Calculating radius\n";
            radius = sequence<size_t>::from_function(srcs.size(), [&](size_t i)
                                                     { return *parlay::max_element(
                                                           parlay::remove_if(
                                                               sequence<size_t>::from_function(G.n,
                                                                                               [&](size_t j)
                                                                                               {
                                                                                                   if (cluster[j] == cluster[srcs[i]])
                                                                                                   {
                                ...
                                sequence<size_t> &r0s)
        {
            ...
            // sequence<size_t> dists;
            BFS_radius(G, srcs, radii, parents, cluster, cluster_index, dists, edges);
            // std::cout << "returned from BFS_radius, srcs.size(): " << srcs.size() << " radii.size(): " << radii.size() << std::endl;
            if (srcs.empty())
            {
                debug(noDoubleEdges(edges););
            ...
            debug(assert(srcs.size() == radii.size()););
            debug(noDoubleEdges(edges););
            ...
            //                                                        { auto r = gen[i]; return (int)floor(radii[i] * distr(r)); });
                                                                   r0s = sequence<size_t>::from_function(srcs.size(), [&](size_t i)
                                                                   { auto r = gen[i]; return (int)floor(radii[i] * distr(r)); });

            debug(assert(r0s.size() == radii.size()););

            debug(assert(inRange(r0s, radii, 0.5, 2.0 / 3.0)););
