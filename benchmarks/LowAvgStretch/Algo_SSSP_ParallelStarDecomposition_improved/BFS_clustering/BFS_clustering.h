...
#include "benchmarks/LowAvgStretch/Algo_SSSP_ParallelStarDecomposition_improved/BallGrowing/BallGrowing.h"
...
namespace gbbs
{
  namespace LowAvgStretch
  {
                               ...
                               double eps)
    {
      ...
      BallGrowing(G, srcs, ball, ballShell, parents, cluster, cluster_index, edges, radius, max_rad, gen, dists, r0s);
      if (srcs.empty())
      {
        // debug(noDoubleEdges(edges););
      ...
      double beta = (c * log(ballShell.size()) / (eps * max_rad));
