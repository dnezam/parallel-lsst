...
TEST(ReadCustomGraph, GML) {
  const std::vector<gi::Edge<NoWeight>> kEdges{
      {1, 5}, {0, 5}, {6, 3}, {2, 0}, {1, 0},
  };
  auto graph{gi::edge_list_to_symmetric_graph(kEdges)};
  auto g = std::get<1>(gbbs_io::read_unweighted_symmetric_graph_custom("inputs/test_graph/tiny_py_GML"));
  EXPECT_EQ(g.n, 7);
  EXPECT_EQ(g.m, 10);
}

TEST(ReadCustomGraph, BIN) {
    const std::vector<gi::Edge<NoWeight>> kEdges{
      {1, 5}, {0, 5}, {6, 3}, {2, 0}, {1, 0},
  };
  auto graph{gi::edge_list_to_symmetric_graph(kEdges)};
  auto g = std::get<1>(gbbs_io::read_unweighted_symmetric_graph_custom("inputs/test_graph/tiny_py_BIN"));
  EXPECT_EQ(g.n, 7);
  EXPECT_EQ(g.m, 10);
}

TEST(WriteCustomGraph, GML) {
    const std::vector<gi::Edge<NoWeight>> kEdges{
      {1, 5}, {0, 5}, {6, 3}, {2, 0}, {1, 0},
  };
  auto graph{gi::edge_list_to_symmetric_graph(kEdges)};
  auto g = gi::gbbs_2_nk(graph);
  gbbs_io::write_unweighted_symmetric_graph_custom("inputs/test_graph/tiny_cpp_GML", g);
  // testing in python
}

TEST(WriteCustomGraph, BIN) {
    const std::vector<gi::Edge<NoWeight>> kEdges{
      {1, 5}, {0, 5}, {6, 3}, {2, 0}, {1, 0},
  };
  auto graph{gi::edge_list_to_symmetric_graph(kEdges)};
  auto g = gi::gbbs_2_nk(graph);
  gbbs_io::write_unweighted_symmetric_graph_custom("inputs/test_graph/tiny_cpp_BIN", g);
  // testing in python
}
