...
TITLE = "$What with size $Size"
Y_LABEL = "$What vs number of processors"
Y_LIMIT = False
...
USE_BOX = True
PROCESSOR_INTERVAL = 4
PROCESSOR_MAX = 120
PROCESSOR_SWITCH = 12
PROCESSOR_LINEAR_MAX = 11
...
# The most difference in SparseErdosRenyi for Stretch
...
TITLE = TITLE.replace('$Size', str(graph_size))
TITLE = TITLE.replace('$What', GRAPH_TYPE)



DIMENSIONS = [1, 2, 3, 4, 5, 6, 7, 8, 16, 32]#, 64, 128]
MACHINE_SPEC = "32-Core AMD EPYC 7R13 3.6 GHz"#"Intel i7-i7-8650U, 1.9 GHz (Kaby Lake)"
FONT_SIZE = 12
MARKERSIZE = 4
FONT_WEIGHT = 'normal'
if PLOT_STRETCH:
    NAME_OVERWRITE = f'{GRAPH_TYPE}_{graph_size}_stretch'
else:
    NAME_OVERWRITE = f'{GRAPH_TYPE}_{graph_size}'
NAME_OVERWRITE = ''
...
def create_plot():
    ...
    for alg in par_algos:
        data = []
        CIS = []
        # positions = list(range(1, PROCESSOR_LINEAR_MAX+1))
        # positions = DIMENSIONS
        positions = [i for i in range(len(DIMENSIONS))]
        for num_processors in DIMENSIONS:
            df = read_performance_data(alg, f'{GRAPH_TYPE}/{graph_size}', num_processors)
            if PLOT_STRETCH:
    ...
    for alg in seq_algos:
        ...
        CIS = []
        positions = [0]
        df = read_performance_data(alg, f'{GRAPH_TYPE}/{graph_size}', 1)
        if PLOT_STRETCH:
            df = df.drop('Runtime', axis=1)
        else:
            df = df.drop('AvgStretch', axis=1)
        ...
        boxes = p.boxplot(data, notch=True, positions=positions, manage_ticks=False, conf_intervals=CIS, sym='', widths=0.5)
        # print(len(boxes))
        colors.append(boxes['boxes'][0])
    ...
    xlabels = [f'{DIMENSIONS[x] if (x % 1 == 0) else ""}' for x in p.get_xticks()]
    ...
    if len(NAME_OVERWRITE) == 0:
        output_file = f'{output_folder}/{GRAPH_TYPE}{"-stretch" if PLOT_STRETCH else ""}-{plot_name}.{"eps" if SAVE_AS_EPS else "png"}'
    else:
